#Creates a function to simulate the pediatric (EPI) vaccine outputs
simulate_epi <- function(birth_cohort, periods, h, mu_ac, mu_gnr, n_simulations, coverage_epi, efficacy_epi) {
  ...
  for (sim in 1:n_simulations) {
    ...
    for (age in 1:periods) {
      if (age == 1) {
        ...
        hospitalizations_unvax_sim[sim, age] <- remaining_cohort*h_vec_sim[sim,age]
        ...
        unvax_other_cause_deaths_sim[sim, age] <- remaining_cohort*mu_ac[age] - unvax_deaths_due_to_gnr_sim[sim, age]
        ...
        survived_vaxed_sim[sim, age] <- survived_sim[sim, age]*coverage_epi
        survived_unvax_sim[sim, age] <- survived_sim[sim, age]*(1 - coverage_epi)
  ...
  epi_hosp_cost <- total_hospitalizations_sim*157.50
  epi_cost_per_dose <- 1.47
  epi_doses_per_child <- 2
  ...
  #COMMENT IN/OUT DESIRED DISCOUNTED OR UNDISCOUNTED DALYS
  daly_avert_epi_sq <- sq_daly_disc - total_daly_discounted_sim #only used for discounted DALYs
  #daly_avert_epi_sq <- sq_daly_undisc - total_daly_sim
  #daly_avert_epi_mat <- mat_daly_undisc - total_daly_sim
  daly_avert_epi_mat <- mat_daly_disc - total_daly_discounted_sim
  icer_epi_sq <- incr_cost / daly_avert_epi_sq
  icer_epi_mat <- incr_cost_epi_mat / daly_avert_epi_mat
  
  #Comparing EPI to Status Quo
  expected_daly_avert_epi_sq <- mean(daly_avert_epi_sq)
  daly_avert_epi_sq_ci <- ci(daly_avert_epi_sq)
  expected_icer_epi_sq <- mean(incr_cost)/expected_daly_avert_epi_sq
  icer_epi_sq_ci <- ci(icer_epi_sq)
  
  #Comparing EPI to Maternal
  expected_daly_avert_epi_mat <- mean(daly_avert_epi_mat)
  daly_avert_epi_mat_ci <- ci(daly_avert_epi_mat)
  expected_icer_epi_mat <- median(icer_epi_mat)
  icer_epi_mat_ci <- ci(icer_epi_mat)
  
  icer_matrix_epi_sq <-cbind(incr_cost, daly_avert_epi_sq, icer_epi_sq)
  icer_matrix_epi_mat <- cbind(incr_cost_epi_mat, daly_avert_epi_mat, icer_epi_mat)
  
  #Reduced hosp and death, Figure 2
  ...
  # OUTPUT ICER DATA
  return(data.frame(daly_avert_epi_sq = expected_daly_avert_epi_sq, daly_epi_sq_low = daly_avert_epi_sq_ci[1], daly_epi_sq_high = daly_avert_epi_sq_ci[2],
                    icer_epi_sq = expected_icer_epi_sq, icer_epi_sq_low = icer_epi_sq_ci[1], icer_epi_sq_high = icer_epi_sq_ci[2],
                    daly_avert_epi_mat = expected_daly_avert_epi_mat, daly_epi_mat_low = daly_avert_epi_mat_ci[1], daly_epi_mat_high = daly_avert_epi_mat_ci[2],
                    icer_epi_mat = expected_icer_epi_mat, icer_epi_mat_low = icer_epi_mat_ci[1], icer_epi_mat_high = icer_epi_mat_ci[2]))
  
  # OUTPUT ICER MATRIX - EPI SQ
  #return(icer_matrix_epi_sq)
  
  # OUTPUT ICER MATRIX - EPI MAT
  #return(icer_matrix_epi_mat)
  
  #FOR COVERAGE ANALYSIS, FIGURE 2
  #return(data.frame(coverage = coverage_epi, hosp_n = expected_reduced_hosp, hosp_low = reduced_hosp_ci[1], hosp_high = reduced_hosp_ci[2],
...
#generate data- comment in/out desired output of function
#TO OUTPUT PEDIATRIC DATA
#epi_data <- simulate_epi(birth_cohort, periods, h, mu_ac, mu_gnr, n_simulations, coverage_epi, efficacy_epi)

#TO OUTPUT MATRIX OF HOSPITALIZATIONS, DEATHS, AND DALYS LOST
#epi_hosps_deaths_dalys <- simulate_epi(birth_cohort, periods, h, mu_ac, mu_gnr, n_simulations, coverage_epi, efficacy_epi)

#TO OUTPUT ICER DATA
epi_icer_data <- simulate_epi(birth_cohort, periods, h, mu_ac, mu_gnr, n_simulations, coverage_epi, efficacy_epi)

#TO OUTPUT ICER MATRIX - EPI SQ
#epi_sq_icer_matrix <- simulate_epi(birth_cohort, periods, h, mu_ac, mu_gnr, n_simulations, coverage_epi, efficacy_epi)

#TO OUTPUT ICER MATRIX - EPI MAT
#epi_mat_icer_matrix <- simulate_epi(birth_cohort, periods, h, mu_ac, mu_gnr, n_simulations, coverage_epi, efficacy_epi)

#FIGURE 2 MATERNAL COVERAGE
#epi_coverage_change <- simulate_epi(birth_cohort, periods, h, mu_ac, mu_gnr, n_simulations, coverage_epi, efficacy_epi)


#EXPORT PEDIATRIC DATA
#library("writexl")
#write_xlsx(epi_data,"C:/Users/wstil/OneDrive/Desktop/Aim 3/Hosp Deaths DALY Figure/epi_output.xlsx", col_name=TRUE, format_headers=FALSE)